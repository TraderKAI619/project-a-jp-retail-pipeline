...
**課題**  
日本の小売業において、ゴールデンウィークなどの大型連休時に需要変動を予測し、  
在庫や人員配置を最適化することが長年の課題となっている。

**解決策**  
- Medallion アーキテクチャに基づくバッチ型データパイプラインを構築  
- 政府公開データ（祝日、JIS 地域コード、消費税履歴）を統合  
- 地域別・商品カテゴリ別の上振れ率（Uplift）分析を自動化し、意思決定者に可視化

**成果**  
- データ品質検証率 約95%（Project B で確立した DQ フレームワークを継承）  
- 消費税変更履歴を SCD Type 2 で管理し、税制改正影響を追跡可能  
- 合成データにより、商用データを使用せずに再現可能な分析環境を構築（MIT License）

**Why it matters**  
本プロジェクトは「技術的に正確な ETL」だけでなく、  
「日本市場特有の商業課題をデータで解決する力」を実証するためのものです。  
